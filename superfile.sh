...
if command -v curl &> /dev/null; then
    echo -e "${bright_yellow}Descargando ${cyan}${package} v${version} for ${os} (${arch})...${nc}"
    curl -sLO "$url"
else
    echo -e "${bright_yellow}Descargando ${cyan}${package} v${version} for ${os} (${arch})...${nc}"
    wget -q "$url"
...
echo -e "${bright_yellow}Extrayendo ${cyan}${package}...${nc}"
tar -xzf "${file_name}.tar.gz"

echo -e "${bright_yellow}Instalando ${cyan}${package}...${nc}"
cd ./dist/${file_name}
chmod +x ./spf
...
if ! sudo mv ./spf /usr/local/bin/; then
  ...
  echo -e "${bright_green}Instalación Completada!${nc}"
fi
